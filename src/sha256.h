...
public:
  SHA256(std::string Data)
      : SHA256(reinterpret_cast<const byte_t *>(Data.data()), Data.size()) {}

  SHA256(const char *Data)
      : SHA256(reinterpret_cast<const byte_t *>(Data), strlen(Data)) {}

  SHA256(const byte_t *Data, size_t Len) {
    std::vector<byte_t> PaddingData = padding(Data, Len);

    for (size_t i = 0; i < PaddingData.size(); i += 64) {
      std::vector<byte_t> Section(PaddingData.begin() + i,
                                  PaddingData.begin() + i + 64);
      std::vector<word_t> W = byteToWord(Section);
      update(W);
    }
  }
...
private:
  void update(std::vector<word_t> &W) {
    ...
    auto op_s0 = [this](word_t x) {
      return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22);
    };

    auto op_s1 = [this](word_t x) {
  ...
  std::vector<word_t> byteToWord(const std::vector<byte_t> &Section) {
    std::vector<word_t> W(64, 0);

    // Load the first 16 words from the data section
    for (size_t i = 0; i < 16; ++i)
      for (size_t j = 0; j < 4; ++j)
        W[i] |= (static_cast<word_t>(Section[i * 4 + j]) << (24 - j * 8));
    ...
    for (size_t j = 16; j < 64; ++j) {
      word_t s0 = op_s0(W[j - 15]);
      word_t s1 = op_s1(W[j - 2]);
  ...
  word_t rotr(word_t Value, int Shift) const {
  ...
  std::vector<byte_t> padding(const byte_t *Data, size_t Len) {
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      PaddingData[PaddingLen - 1 - i] = BitsLen & 0xff;
      BitsLen >>= 8;
    }
    return PaddingData;
  }
